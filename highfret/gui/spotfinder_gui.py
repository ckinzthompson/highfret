...
def gui_spotfinder(fn_data='',fn_align='',fn_cal=''):
	out = widgets.Output()
	...
	accordion = widgets.Accordion(children=[
		widgets.VBox([text_data_filename,text_align_filename,text_calibration_filename,]),
		widgets.VBox([dropdown_split,int_acf_start_g,int_acf_end_g,int_acf_start_r,int_acf_end_r,]),
		widgets.VBox([dropdown_localmax,float_smooth,dropdown_which,dropdown_matched,float_acf_cutoff,checkbox_refine,]),
		], titles=('Files','ACF','Spotfinding',))

	# vbox_spots = widgets.VBox([accordion_files,accordion_acf,accordion_spots,button_prepare,button_spotfind,])
	title = widgets.HTML(value="<h3>Spotfinding</h3>")
	vbox_spots = widgets.VBox([title,accordion,button_prepare,button_spotfind,])
